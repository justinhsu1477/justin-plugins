...
- **YES** → write ONE focused **failing** test that pins the intended behavior (include an edge case), then implement until it goes green. With AI, let it draft the test, but **you review the cases for meaningfulness** — a test that doesn't assert the right thing is worse than none.
